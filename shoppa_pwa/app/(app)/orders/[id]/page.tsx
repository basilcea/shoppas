...
interface CompletedItem extends BaseItem {
  completed: true
}

interface UnavailableItem extends BaseItem {
  unavailable: true
}

type OrderItem = BaseItem | CompletedItem | UnavailableItem
           ...
           <section key={category}>
             ...
             <div className="space-y-3">
               {items.map((item, idx) => {
                 ...
                 const isUnavailable = 'unavailable' in item && item.unavailable
                 return (
                   <div
                     key={idx}
                     className={`bg-surface-container-lowest p-4 rounded-xl shadow-sm ${
                       isCompleted ? 'border-l-4 border-secondary' : isUnavailable ? 'border-l-4 border-error' : ''
                     }`}
                   >
                     <div className="flex justify-between items-start mb-2">
                       <div className="flex items-center gap-2">
                         <h3 className={`font-headline font-bold text-base ${isCompleted ? 'text-on-surface-variant line-through' : 'text-on-surface'}`}>
                           {item.name}
                         </h3>
                         {isCompleted && (
                       ...
                       <span className={`font-bold text-sm ${isUnavailable ? 'text-error' : 'text-primary'}`}>
